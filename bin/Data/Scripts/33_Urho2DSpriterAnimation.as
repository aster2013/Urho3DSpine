...
//     - Creating a 2D scene with spine and spriter animation
...
void CreateScene()
{
    ...
    AnimationSet2D@ spineAnimationSet = cache.GetResource("AnimationSet2D", "Urho2D/spineboy/spineboy.json");
    if (spineAnimationSet is null)
        return;

    spineNode = scene_.CreateChild("spineAnimation");
    spineNode.position2D = Vector2(-2.0f, -1.0f);
...
{
    AnimatedSprite2D@ spineAnimatedSprite = spineNode.GetComponent("AnimatedSprite2D");
    AnimationSet2D@ spineAnimationSet = spineAnimatedSprite.animationSet;
    spineAnimationIndex = (spineAnimationIndex + 1) % spineAnimationSet.numAnimations;
